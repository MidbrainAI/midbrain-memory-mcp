...
mcp.mjs                          All 7 MCP tool definitions (createServer factory)
...
shared/
  ...
  pk-inject.mjs                  PK injection helpers: extractInjectedPkIds,
                                 formatPkContext, stripInjectedContext. Shared by all
                                 capture paths. Marker format mirrors litellm-proxy.
  logger.mjs                     makeDebugLogger() — timestamped file appender
  plugin-entry.mjs               esbuild entry point → dist/midbrain-shared.mjs bundle
  clients/
...
plugins/
  opencode/
    midbrain-memory.ts           OpenCode plugin (TypeScript, Bun runtime). Captures
                                 every message via chat.message hook. Injects PK
                                 context block into user message parts before LLM.
                                 Full cross-turn dedup via session history.
  claude-code/
    ...
    capture-user.mjs             Hook: UserPromptSubmit → storeEpisodic("user") +
                                 searchProcedural → additionalContext injection
  ...
  codex/
    ...
    capture-user.mjs             Hook: UserPromptSubmit → storeEpisodic("user") +
                                 searchProcedural → hookSpecificOutput injection
...
At install time, this file and `dist/midbrain-shared.mjs` (esbuild bundle of
all `shared/` modules) are copied into `~/.config/opencode/plugins/`. The plugin
imports from `./midbrain-shared.mjs` at runtime.
...
On every user turn, all three capture paths automatically search for relevant
procedural knowledge (PK) entries via `api.searchProcedural()` (2s hard timeout
via `AbortSignal.timeout` — silent fallthrough on timeout or any error).

If matches are found, a context block is prepended to the user message before
the LLM sees it:

```
<!-- mb:ctx-start -->
## Procedural knowledge:
<!-- mb:ctx-meta nonce=<runtime-nonce> sig=<id-signature> -->
<!-- mb:pk 1,3 -->
### Title
content
...
The `<!-- mb:pk id,... -->` marker enables session-level deduplication: ids
already injected in prior turns are passed as `exclude_ids` to the search so
each entry appears at most once per session.

Trusted injected blocks also include `ctx-meta nonce` metadata plus a
process-secret signature over the PK ids. Dedup trusts only blocks generated by
the current hook runtime; user-authored marker examples, copied blocks with
altered ids, and unsigned full context blocks are ignored. PK payloads are
capped at 160 title characters, 2,000 content characters per entry, and 6,000
characters total, with marker-like text escaped before injection.

**Dedup availability by client:**
- OpenCode: full cross-turn dedup — session messages are fetched via the SDK
- Claude Code / Codex: no conversation history in hook payload → `exclude_ids=[]`
  The same entry may recur across turns; `min_score=0.5` limits this to
  relevant matches only.

The public `mb:pk` marker remains compatible with litellm-proxy, but this
package only trusts markers inside signed `ctx-meta nonce` blocks for dedup.

**Helpers** live in `shared/pk-inject.mjs`:
- `extractInjectedPkIds(texts)` — scans prior message texts for `mb:pk` markers
- `formatPkContext(entries)` — builds the `mb:ctx-start/end` block
- `stripInjectedContext(text)` — removes the block (prevents accumulation)
...
- `GET /api/v1/memories/search/procedural?query=...&limit=5&min_score=0.5&exclude_ids=1&exclude_ids=2`
  Returns: `[{id, title, content, source_ids, score, created_at, updated_at}]`
  Semantically searches procedural knowledge entries. `exclude_ids` is a repeated
  param; pass ids from prior injections to skip already-seen entries (session dedup).
  `min_score` default 0.5 suppresses off-topic queries. Called automatically by all
  capture hooks — do NOT call this manually from application code.
...
PK injection: after storing episodic, call api.searchProcedural() and if
matches are found prepend a ctx block to output.parts[0].text before
returning. Use client.session.messages() to extract excludeIds for dedup.
...
At install time, the following are copied to ~/.config/opencode/plugins/:
  ...
  dist/midbrain-shared.mjs             →  ~/.config/opencode/plugins/midbrain-shared.mjs

The bundle (`dist/midbrain-shared.mjs`) is built by esbuild from
`shared/plugin-entry.mjs` and includes all shared modules:
  midbrain-api.mjs, pk-inject.mjs, logger.mjs, clients/*.mjs

The plugin imports from `./midbrain-shared.mjs` at runtime. Both files must
be present. Run `npm run build:plugin` to rebuild the bundle after changing
any `shared/` file. The `npm run check` and `npm run bootstrap` commands
include this build step automatically.
...
- Procedural knowledge (PK) is injected automatically into each user message
  by the capture hooks before the LLM sees it. Do NOT manually search or list
  PK — it is not exposed as an MCP tool.
